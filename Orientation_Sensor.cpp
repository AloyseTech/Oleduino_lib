...
{
        I2CwriteByte(ACCEL_GYRO_ADDRESS, subAddress, data);
}

void LSM9DS1::mWriteByte(uint8_t subAddress, uint8_t data)
{
        return I2CwriteByte(MAGNETOMETER_ADDRESS, subAddress, data);
}

uint8_t LSM9DS1::xgReadByte(uint8_t subAddress)
{
        return I2CreadByte(ACCEL_GYRO_ADDRESS, subAddress);
}

void LSM9DS1::xgReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
        I2CreadBytes(ACCEL_GYRO_ADDRESS, subAddress, dest, count);
}

uint8_t LSM9DS1::mReadByte(uint8_t subAddress)
{
        return I2CreadByte(MAGNETOMETER_ADDRESS, subAddress);
}

void LSM9DS1::mReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
        I2CreadBytes(MAGNETOMETER_ADDRESS, subAddress, dest, count);
}


void LSM9DS1::initI2C()
{
    //Wire.begin();	// Initialize I2C library !!! already done
}
